		...
		|																|
		|	Version 3:	Improved multithreaded.	 Addresses issues		|
		|				in V. 2 regarding passing of data and thread	|
		|				creation.										|
		|																|
		|	Note:	I removed some of the long explanations from V2		|
...
//	computed by that thread.  Look at the Thread 2 code sample explanation
...
void* threadFunc(void* argument)
{
	...
	double* myXY = xy + 2*POINTS_PER_THREAD*(info->threadIndex);
	
	//new
	//	This will count the number of points below the graph
	//	Note that I don't need to initialize to 0 here since this was done
	//	in the main thread prior to calling pthread_create
	//info->counter = 0;
